...
Normal progression is event-driven: each successful stage explicitly dispatches its successor, failed/no-branch implementations retry themselves immediately, and successful merges explicitly close their linked issue before the orchestrator selects more work. Every unsuccessful terminal pipeline workflow wakes a separate watchdog. Scheduled orchestration and reconciliation are backup signals only; correctness must not depend on GitHub delivering a cron event. The reconciler also recovers transient missed dispatches, stale labels, and open issues already satisfied by merged PRs. Implementation output is retained in the Actions log, and each issue receives at most three implementation attempts before `automation:needs-attention` is applied. After three failed implementation, CI, or review-repair cycles, resolve the substantive blocker, remove `automation:needs-attention`, add `automation:retry`, and dispatch the reconciler or orchestrator. Never bypass failed custody, security, CI, or Claude-review gates merely to advance the queue.
