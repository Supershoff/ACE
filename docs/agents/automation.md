...
2. `claude.yml` first recovers any existing branch whose tip canonically closes the issue and is ahead of `master`, then aliases a nonstandard recovered name into the pipeline's issue-scoped branch namespace. Only when no recoverable branch exists does Claude implement from `master`, follow Red → Green → Refactor, and hand the branch to the wrapper, which opens a draft pull request.
...
- `automation:needs-attention` — three substantive implementation failures exhausted bounded recovery or a real human decision is required. Provider/API outages, incomplete background-task handoffs, and failures with a recoverable branch do not consume this budget.
...
Normal progression is event-driven: each successful stage explicitly dispatches its successor, failed/no-branch implementations retry themselves immediately, and successful merges explicitly close their linked issue before the orchestrator selects more work. Before spending another agent run, the implementation workflow searches all repository branches for a unique, ancestry-valid tip with a canonical `Fixes`, `Closes`, or `Resolves` reference to the issue; branch prefixes are conventions, not recovery boundaries. Every unsuccessful terminal pipeline workflow wakes a separate watchdog. Scheduled orchestration and reconciliation are backup signals only; correctness must not depend on GitHub delivering a cron event. The reconciler also recovers transient missed dispatches, stale labels, and open issues already satisfied by merged PRs. Implementation output is retained in the Actions log. Only a completed agent run that produces neither a valid branch nor a classified transient provider/orchestration failure consumes the three-attempt substantive implementation budget. After three substantive implementation, CI, or review-repair failures, resolve the blocker, remove `automation:needs-attention`, add `automation:retry`, and dispatch the reconciler or orchestrator. Never bypass failed custody, security, CI, or Claude-review gates merely to advance the queue.
