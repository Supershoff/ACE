...
public sealed class CloudCustodyBoundary
{
    ...
    /// <summary>
    /// Opens ACE's local authority record for a new Withdrawal Token's exclusive Withdrawal
    /// Reservation over a whole (non-stack) Cloud Item (WDR-001, WDR-002). Reuses
    /// <see cref="CloudReservationPolicy.Open"/> (ACE.Cloud.Domain) for the exclusivity decision so
    /// this shares the exact same rule the companion backend's own reservation workflows will use.
    /// Repeating this call with the same <paramref name="idempotencyKey"/> replays the original
    /// committed reservation (transaction rule 4). Scope: whole-item reservations only; a Cloud
    /// Stack Lot withdrawal reservation follows the same shape in a later issue.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return CloudBoundaryRetry.ExecuteAsync(
            () => TryReserveForWithdrawalOnceAsync(biotaId, shardId, ownerId, tokenHash, timeToLive, idempotencyKey, cancellationToken),
            cancellationToken: cancellationToken);
    }
    ...
    /// <summary>
    /// Cancels an active Withdrawal Reservation before redemption (WDR-003). Idempotent by
    /// construction rather than by a stored idempotency key: cancelling an already-cancelled
    /// reservation is a no-op success, while cancelling one already released for a different reason
    /// (for example already redeemed) is a Conflict.
    /// </summary>
    ...
    /// <summary>
    /// Reads whether a Withdrawal Token's local reservation is currently active without consuming
    /// it (WDR-008: ACE validates an already-issued token's local reservation and redemption rules
    /// even while the companion web service is unreachable). Returns null when no active
    /// reservation matches <paramref name="tokenHash"/> -- either none was ever opened, or it was
    /// already released.
    /// </summary>
    ...
    /// <see cref="WithdrawAsync"/> and releases the reservation as fulfilled, in one transaction, so
    /// the reservation can never observably outlive (or be released without) its custody transition
    /// (WDR-001, WDR-003). Refuses an expired or already-released reservation instead of redeeming
    /// it. Repeating this call with the same <paramref name="idempotencyKey"/> replays the original
    /// committed result (transaction rule 4).
    /// </summary>
    public Task<CloudBoundaryOutcome<CloudWithdrawalResult>> RedeemWithdrawalReservationAsync(
        string tokenHash,
        uint recipientContainerId,
        Guid idempotencyKey,
        CancellationToken cancellationToken = default)
    {
        ...
        return CloudBoundaryRetry.ExecuteAsync(
            () => TryRedeemWithdrawalReservationOnceAsync(tokenHash, recipientContainerId, idempotencyKey, cancellationToken),
            cancellationToken: cancellationToken);
    }
    ...
    /// <paramref name="idempotencyKey"/>, or null if none has committed yet (transaction rule 8).
    /// </summary>
    public async Task<CloudBoundaryOutcome<CloudWithdrawalResult>?> TryGetWithdrawalRedemptionOutcomeAsync(
        Guid idempotencyKey, CancellationToken cancellationToken = default)
    {
        var existing = await FindIdempotencyRecordAsync(idempotencyKey, cancellationToken);
        ...
        uint biotaId, string shardId, Guid ownerId, string tokenHash, TimeSpan timeToLive, Guid idempotencyKey, CancellationToken cancellationToken)
    {
        ...
        // Locking the custody record row serializes every concurrent Reserve attempt for the same
        // biota, which is what makes the exclusivity check below race-free without needing a
        // partial-unique index (INV-001: "One quantity may have at most one exclusive reservation").
        ...
        if (record.IsStack)
        {
            await transaction.RollbackAsync(cancellationToken);
            return CloudBoundaryOutcome<CloudWithdrawalReservation>.Conflict(
                $"Biota {biotaId} is a stack Cloud Custody Record; reserve a Cloud Stack Lot instead.");
        }
        ...
        var activeReservation = await _context.CloudWithdrawalReservations.AsNoTracking()
            .Where(r => r.BiotaId == biotaId && r.Status == CloudReservationStatus.Active)
        ...
        var reservation = CloudWithdrawalReservation.Open(
            shardId, biotaId, ownerId, tokenHash, idempotencyKey,
        ...
        catch (DbUpdateException ex) when (IsDuplicateKey(ex))
        {
            ...
            return CloudBoundaryOutcome<CloudWithdrawalReservation>.Conflict(
                $"A Withdrawal Reservation for biota {biotaId} or this Withdrawal Token already exists.");
        }

        var correlationId = Guid.NewGuid();
        await AppendLedgerAndOutboxAsync(
            correlationId, shardId, CloudBoundaryOperationType.WithdrawalReservationOpened, biotaId, ownerId, faultInjector: null, cancellationToken);
        ...
        Guid reservationId, int expectedVersion, CancellationToken cancellationToken)
    {
        ...
        await AppendLedgerAndOutboxAsync(
            ...
            reservation.BiotaId, reservation.OwnerId, faultInjector: null, cancellationToken);
    ...
    private async Task<CloudBoundaryOutcome<CloudWithdrawalResult>> TryRedeemWithdrawalReservationOnceAsync(
        string tokenHash, uint recipientContainerId, Guid idempotencyKey, CancellationToken cancellationToken)
    {
        _context.ChangeTracker.Clear();
        ...
        {
            return ReplayWithdrawalReservationRedemption(existingIdempotency);
        }
        ...
        if (reservation is null)
        {
            ...
            if (winner is not null)
            {
                return ReplayWithdrawalReservationRedemption(winner);
            }

            return CloudBoundaryOutcome<CloudWithdrawalResult>.Conflict("No Withdrawal Reservation matches this Withdrawal Token.");
        }
        ...
        var record = await LockCustodyRecordByBiotaIdAsync(reservation.BiotaId, cancellationToken);
        if (record is null)
        {
            await transaction.RollbackAsync(cancellationToken);
            return CloudBoundaryOutcome<CloudWithdrawalResult>.Conflict(
                $"Biota {reservation.BiotaId} no longer has a Cloud Custody Record to withdraw.");
        }
        ...
        await AppendLedgerAndOutboxAsync(
            correlationId, shardId, CloudBoundaryOperationType.Withdrawal, biotaId, ownerId, faultInjector: null, cancellationToken);
        ...
        _context.CloudIdempotencyRecords.Add(
            new CloudIdempotencyRecord(
                idempotencyKey, shardId, CloudBoundaryOperationType.WithdrawalReservationRedeemed, biotaId, ownerId,
                custodyRecordId: record.Id, targetContainerId: recipientContainerId, correlationId));
        await _context.SaveChangesAsync(cancellationToken);
    ...
    private static CloudBoundaryOutcome<CloudWithdrawalResult> ReplayWithdrawalReservationRedemption(CloudIdempotencyRecord existing)
    {
