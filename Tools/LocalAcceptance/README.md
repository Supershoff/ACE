...
## 6. Stage and activate your own client_portal.dat

Deposited items only get a real reconstructed icon once a `client_portal.dat` has been staged and
activated for this shard. With the stack from step 5 still running:

```powershell
./Add-LocalAcceptancePortalDat.ps1 -ClientPortalDatPath "C:\path\to\your\client_portal.dat"
```

This drives the existing chunked-upload/finalize Asset Import API against your own local file: it
copies the bytes into this stack's disposable `.local-run/protected-assets/` storage, waits for the
already-running `ACE.Cloud.Worker` process to extract them, and activates the resulting manifest.
Supports a stock or a modified ACE-compatible DAT. Never upload/commit the DAT itself, and never
hard-code a personal path -- always pass your own via `-ClientPortalDatPath`. Once active, the
[icon composition worker](../../Source/ACE.Cloud.Worker/CloudIconCompositionWorker.cs) composes icons
...
- [ ] Confirm a visible, accessible "Log out" control returns you to the Login page and a subsequent login works normally.
